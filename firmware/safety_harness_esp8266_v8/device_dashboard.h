...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>v8.0.0</title>
<style>
...
.case{padding:20px 16px;text-align:center;margin-bottom:16px;border-left:6px solid var(--mut)}.case .lbl{font-size:12px;letter-spacing:1px;color:var(--mut)}.case .cv{font-size:32px;font-weight:700;margin:4px 0}.case .cs{font-size:13px;font-weight:700;letter-spacing:1px}.case.safe{border-left-color:var(--ok)}.case.safe .cs{color:var(--ok)}.case.unsafe{border-left-color:var(--bad)}.case.unsafe .cs{color:var(--bad)}
@media(prefers-reduced-motion:reduce){.fl{transition:none}}
</style></head><body>
<header><div class="dot" id="conn" aria-label="Connection status"></div><h1>v8.0.0</h1><button id="lightToggle" class="tg" style="margin-left:auto" disabled>LIGHT MODE</button></header>
<div class="wrap">
...
<div class="box case" id="caseBox" role="status" aria-live="polite"><div class="lbl">HOOK CASE</div><div class="cv" id="caseV">Detecting</div><div class="cs" id="caseS">-</div><p class="hint" id="caseBase">Free baseline: learning…</p><div class="row" style="justify-content:center"><button id="caseRelearn">RELEARN FREE BASELINE</button></div><p class="hint" id="caseX">Fitted to prototype SBOX-2026-9509, LOW guard by hook mode. Electrical reading only; does not prove the gate is closed.</p></div>
...
<p class="hint">The hook alarm requires BOTH hooks inside either of their own ranges; they may be in different ranges. Endpoints are inclusive, in raw discharge units (0–1,000,000). Range 1 must end before range 2 starts. Defaults: 100–2,230 and 3,000–1,000,000 for each hook (fitted to SBOX-2026-9509, LOW guard by hook).</p>
...
<p class="hint">Up to five networks. Saved passwords remain visible here. Default is tried first after startup; automatic retries continue while the hotspot is in use. SET DEFAULT also enables auto-connect. Open this page manually at http://192.168.4.1/.</p>
...
if(d.hook_case){const known=d.hook_case!=='DETECTING';$('caseV').textContent=d.hook_case_label;$('caseS').textContent=!known?'-':d.hook_case_anchored?'ANCHORED':'NOT ANCHORED';$('caseBox').className='box case '+(!known?'':d.hook_case_anchored?'safe':'unsafe');$('caseBase').textContent=d.case_base_ready?'Free baseline A '+d.case_base_a+' · B '+d.case_base_b:'Free baseline: learning (keep both hooks free 5 s). Body case disabled until learned.'}
...
$('caseRelearn').onclick=()=>deviceFetch('/case-baseline',{method:'POST'});
...
const h=['Sl.No','Timestamp','Label','HookA_mean','HookB_mean','A_p2p','B_p2p','Mutual','LinkIdx','LoadA','LoadB','HookA_st','HookB_st','State','Buckle1','Buckle2','Buckle3','HookA_valid','HookB_valid','Mutual_valid','A_timeouts','B_timeouts','SampleSeq','SampleUptime','PredictionEnabled','Prediction','Calibrated','ReferenceMode','SensingMode','SensingName','SensingRevision','HookCase','HookCaseFrame'];
const cell=value=>'"'+String(value??'').replace(/"/g,'""')+'"';const L=[h.map(cell).join(',')];rows.forEach((r,i)=>L.push([i+1,ts(r.t),r.l,r.a,r.b,r.ap,r.bp,r.m,r.k,r.la,r.lb,r.ha,r.hb,r.s,r.b1?'LOCKED':'OPEN',r.b2?'LOCKED':'OPEN',r.b3?'LOCKED':'OPEN',r.av,r.bvalid,r.mv,r.at,r.bt,r.seq,r.uptime,r.predict,r.prediction,r.calibrated,r.reference,r.sensingMode,r.sensingName,r.sensingRevision,r.hc,r.hf].map(cell).join(',')));
...
const wifiText={'startup-wait':'Hotspot ready. Automatic router connection starts about one second after boot.',connected:'Connected to router',connecting:'Connecting (up to 30 seconds)…','retry-wait':'Router unavailable. Retrying saved networks automatically; hotspot stays enabled.',
 'connection-failed':'Router unavailable; retries use 1–5 minute backoff. The hotspot remains available.',
 'hotspot-only':'Auto-connect is disabled. Select a saved network and press SET DEFAULT or CONNECT to enable it.'};
...
async function pollWifi(){
 ...
 try{
  ...
  $('wifiStatus').textContent=wifiSaveError||(!profiles.length?'No saved networks. Save router details to enable auto-connect.':(wifiText[state.status]||'Unknown connection status')+(state.status==='retry-wait'&&state.retry_in_ms?' Next attempt in '+Math.ceil(state.retry_in_ms/1000)+' s.':''));
