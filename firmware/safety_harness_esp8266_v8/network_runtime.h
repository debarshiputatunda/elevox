...
void applyRouterWifi(){
  ...
  // Startup gating is bounded; hotspot clients do not cancel association.
...
void serviceWifi(){
  ...
  if(wifiApplyPending){
    ...
    // Prefer the default again after a lost connection, without waiting for
    // hotspot clients to leave. Failed associations still use bounded backoff.
    wifiRetry.connected();wifiRetry.stamp=now;wifiRetry.waitMs=1000;
...
void showWifiStatus(){
  ...
  uint32_t elapsed=(uint32_t)(millis()-wifiRetry.stamp);
  uint32_t retryIn=wifiRetry.attempting||connected||!netStore.autoConnect||!netStore.count?0:(elapsed<wifiRetry.waitMs?wifiRetry.waitMs-elapsed:0);
  json+=",\"retry_in_ms\":"+String(retryIn)+",\"last_attempt_profile\":"+String(activeNetwork);
  json+=",\"profiles\":[";
