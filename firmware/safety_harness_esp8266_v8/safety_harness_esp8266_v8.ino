// v8.0.0: v7.2 sensing + hook case classifier (hook_case.h) reported in /data.
...
const char* FIRMWARE_VERSION="v8.0.0";
...
bool guardFloat=false; // EEPROM compatibility only; mode selects guard polarity
...
const uint32_t EE_MAGIC=0x5B0C0023; // Retain v6 settings; calibration is isolated per sensing mode.
void loadConfig(){
...
void setupEndpoints(){
  ...
  // Relearn the free baseline used for BODY. Keep both hooks free for 5 s after this.
  server.on("/case-baseline",HTTP_POST,[](){hookCase.relearnBaseline();server.sendHeader("Cache-Control","no-store");server.send(200,"application/json","{\"relearning\":true}");});
  ...
  server.on("/config", [](){
    ...
    if(server.hasArg("guard")){ server.send(409,"text/plain","Use the sensing mode selector"); return; }
  ...
  server.on("/data", [](){
    ...
    snprintf(buf,sizeof(buf),
      "{%s,\"hook_case\":\"%s\",\"hook_case_label\":\"%s\",\"hook_case_id\":%u,\"hook_case_anchored\":%s,\"hook_case_frame\":\"%s\",\"case_base_ready\":%s,\"case_base_a\":%d,\"case_base_b\":%d,\"hook_raw_a\":%d,\"hook_raw_b\":%d,\"sensing_mode\":%u,\"sensing_name\":\"%s\",\"sensing_revision\":%u,\"device_name\":\"%s\",\"light_mode\":%s,\"sample_seq\":%u,\"sample_uptime_ms\":%u,\"sample_age_ms\":%u,\"uptime_ms\":%u,"
      ...
      "\"protocol\":\"elevox-v5/1\",\"firmware\":\"v8.0.0\",\"mutual_valid\":%s,\"a_timeouts\":%u,\"b_timeouts\":%u,\"id\":\"%s\",\"guard\":\"%s\",\"raw1\":%d,\"raw2\":%d,\"a_p2p\":%u,\"b_p2p\":%u,"
      ...
      ranges,hookCaseCode(hookCase.shown()),hookCaseLabel(hookCase.shown()),(unsigned)hookCase.shown(),hookCase.anchored()?"true":"false",hookCaseCode(hookCase.lastFrame()),hookCase.baselineReady()?"true":"false",(int)hookCase.baselineA(),(int)hookCase.baselineB(),
      hookA.valid?(int)hookA.mean:-1,hookB.valid?(int)hookB.mean:-1,
      ...
      sboxID, sensingMode==V6_HIGH?"HIGH":"LOW",
...
void setup(){
  ...
  startHotspot();
  // Try the saved default shortly after boot, including while hotspot clients are connected.
  wifiRetry.stamp=millis();wifiRetry.waitMs=HotspotPolicy::STARTUP_QUIET_MS;
...
void advanceSensing(uint32_t now){
  ...
  if(hookSampler.step()){
    ...
    // Classifier thresholds were fitted to the recorded (smoothed) readings.
    hookCase.update(hookA.valid,hookB.valid,hookA.valid?(int32_t)dispA:-1,hookB.valid?(int32_t)dispB:-1,millis());
    ...
    sensingFrameActive=false;
    static uint32_t lastLog=0;
    if((uint32_t)(sampleStamp-lastLog)>=5000){
      lastLog=sampleStamp;
      Serial.printf("[STREAM] sample=%u frame=%ums A=%d B=%d case=%s heap=%u\n",sampleSequence,frameDuration,hookA.valid?(int)dispA:-1,hookB.valid?(int)dispB:-1,hookCaseCode(hookCase.shown()),ESP.getFreeHeap());
    }
  }
}
