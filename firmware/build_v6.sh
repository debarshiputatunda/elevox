...
sketch="$build_tmp/safety_harness_esp8266_v6"
mkdir -p "$sketch" "$project_root/firmware/releases"
for src in "$project_root/firmware/safety_harness_esp8266_v6/"*.h "$project_root/firmware/safety_harness_esp8266_v6/"*.ino; do
...
python3 - "$build_tmp/build/safety_harness_esp8266_v6.ino.bin" "$project_root/firmware/releases" <<'PY'
...
output=Path(sys.argv[2])/'elevox-v6.1.0.bin'
output.write_bytes(data)
(output.parent/'elevox-v6.1.0.sha256').write_text(hashlib.sha256(data).hexdigest()+'  '+output.name+'\n')
